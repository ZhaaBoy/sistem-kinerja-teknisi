...
@section('content')
    <div class="card bg-base-100 shadow p-6 max-w-2xl">
        ...
        <form method="POST" action="{{ route('barangs.update', $barang) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="keterangan" class="textarea w-full border-gray-300 rounded-lg" rows="3">{{ old('keterangan', $barang->keterangan) }}</textarea>
            </div>
